...
mod tests {
    ...
    fn test_commitment_scheme_verifier_commit() {
        ...
        let commitment_1 = FieldElement::from_hex_be(
        ...
        assert_eq!(
            commitment_scheme.trees[0].column_log_sizes,
            vec![14, 14, 14, 14]
        );

        let commitment_2 = FieldElement::from_hex_be(
        ...
        assert_eq!(
            commitment_scheme.trees[1].column_log_sizes,
            vec![14, 14, 14, 14, 14, 14, 14, 14]
        );

        let commitment_3 = FieldElement::from_hex_be(
        ...
        assert_eq!(
            commitment_scheme.trees[2].column_log_sizes,
            vec![14, 14, 14, 14]
        );
    }
    ...
    fn test_commitment_scheme_verifier_verify_values() {
        ...
        let extended_domain =
            CanonicCoset::new(log_n_rows + config.fri_config.log_blowup_factor + 1).circle_domain();
        ...
        let commitment_scheme_prover_proof =
            commitment_scheme_prover.prove_values(sample_points.clone(), channel);
        ...
        commitment_scheme_verifier.commit(
            commitment_scheme_prover.roots()[0],
            &log_sizes_cols,
            channel,
        );
        ...
        assert!(commitment_scheme_verifier
            .verify_values(sample_points, commitment_scheme_prover_proof, channel)
            .is_ok());
    }

    #[test]
    fn test_commitment_scheme_verifier_verify_values_with_two_columns() {
        let log_n_rows = 3;
        let log_sizes_cols = vec![log_n_rows, log_n_rows];

        let config = PcsConfig {
            pow_bits: 10,
            fri_config: FriConfig::new(0, 2, 1),
        };
        ...
        let circle_evaluation_0 = CircleEvaluation::new(
            domain,
            vec![
                M31::from(0),
                M31::from(1),
        ...
        let circle_evaluation_1 = CircleEvaluation::new(
            domain,
            vec![
                M31::from(0),
                M31::from(1),
        ...
        let trace = vec![circle_evaluation_0, circle_evaluation_1];
        ...
        let sample_points = TreeVec::new(vec![vec![vec![oods_point], vec![oods_point]]]);
