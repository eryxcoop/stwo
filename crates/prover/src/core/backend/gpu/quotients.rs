...
use crate::core::backend::cpu::quotients::{batch_random_coeffs, column_line_coeffs, QuotientConstants};
...
#[allow(unused_imports)]
use crate::core::backend::gpu::{m31::PackedM31, qm31::PackedQM31};
#[allow(unused_imports)]
...
//     sample_batches: &[ColumnSampleBatch],
//     domain: CircleDomain,
// ) -> Vec<Col<GpuBackend, SecureField>> {
...
//                 ))
//             });
//             let domain_points_x = PackedM31::from_array(points.map(|p| p.x).to_vec());
//             let domain_points_y = PackedM31::from_array(points.map(|p| p.y).to_vec());
...
//     <GpuBackend as FieldOps<SecureField>>::batch_inverse(
//         &denominators,
